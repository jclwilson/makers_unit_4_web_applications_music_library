...
class AlbumRepository:
    ...
    def get_album_artist(self, id: int) -> Album | None:
        ...
        row = self._connection.execute(
            "SELECT albums.id AS album_id, albums.title, albums.release_year, albums.artist_id, artists.id, artists.name AS artist_name FROM albums JOIN artists ON albums.artist_id = artists.id WHERE albums.id = %s ORDER BY albums.release_year;",
            [id],
        )
        if len(row) > 0:
            album = Album(
